jQuery( $ => {
    ...
    const nonce = blnotifier_link_search_autocomplete.nonce;
    const ajaxUrl = blnotifier_link_search_autocomplete.ajaxurl;

    const $urlField = $( '#link-search-input' );
    if ( !$urlField.length ) {
        return;
    }
    ...
    const $suggestions = $( '#bln-link-suggestions' );
